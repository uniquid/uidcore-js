    ...
    return new Promise(function (resolve, reject) {
        ...
        function autoloadCallback() {
            ...
            var getLastProviderContractIdentity = function () {
                return (contracts.find({ 'identity.role': Identity_1.Role.Provider, 'identity.ext': '1' }).sort(function (ctr1, ctr2) { return ctr2.identity.index - ctr1.identity.index; })[0] || { identity: { role: Identity_1.Role.Provider, index: -1 } }).identity;
            };
            ...
            var revokeContract = function (revoker) {
                var ctr = contracts.findOne({ revoked: null, revoker: revoker, orchestration: { $ne: true } });
            ...
            var getContractForExternalUser = function (userAddr) {
                return contracts.findOne({ 'identity.role': Identity_1.Role.Provider, contractor: userAddr, revoked: null });
            };
            var findContractsWithUnresolvedProviderNames = function () {
                return contracts.find({ 'identity.role': Identity_1.Role.User, providerName: null });
            };
