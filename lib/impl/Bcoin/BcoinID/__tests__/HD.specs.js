...
describe('HD keys derivation', function () {
    ...
    it('derive ProviderIdentity', function () {
        // m/44'/0'/0/0/1/5
        var identity = HD_1.identityFor(masterPrivKey)({ role: Identity_1.Role.Provider, index: 5, ext: '1' });
        expect(identity.address).toBe('n26AHAeWe9NxTFmZ7DuDtCaBNMzfcKzPRA');
    });
    it('derive UserIdentity', function () {
        // m/44'/0'/0/1/0/5
        var identity = HD_1.identityFor(masterPrivKey)({ role: Identity_1.Role.User, index: 5, ext: '0' });
