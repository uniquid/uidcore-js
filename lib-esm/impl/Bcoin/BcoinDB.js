...
exports.makeBcoinDB = (options) => new Promise((resolve, reject) => {
    ...
    function autoloadCallback() {
        ...
        const getLastProviderContractIdentity = () => (contracts.find({ 'identity.role': Identity_1.Role.Provider, 'identity.ext': '1' }).sort((ctr1, ctr2) => ctr2.identity.index - ctr1.identity.index)[0] || {
        ...
        const getContractForExternalUser = (userAddr) => contracts.findOne({
            'identity.role': Identity_1.Role.Provider,
            contractor: userAddr,
            revoked: null
        });
        const findContractsWithUnresolvedProviderNames = () => contracts.find({
